...
def get_subjects_info_api_call(api_key, department_code):
    endpoint = "https://gw.its.yale.edu/soa-gateway/courses/webservice/v3/index"
    api_url = f"{endpoint}?apikey={api_key}&subjectCode={department_code}"
    response = requests.get(api_url)
...
def get_detailed_descriptions(api_key):

    subjects = get_subjects_api_call(api_key)
    ...
    for department_code in unique_department_codes:
        department_sujbects_info = get_subjects_info_api_call(api_key, department_code)
